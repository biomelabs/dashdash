...
function syncScrubControls(): { viewStart: number; followTail: boolean } {
  const result = updateScrubControls(
    viewStart,
    followTail,
    VIEW_WINDOW_SECONDS,
  );
...
async function exportCsv(): Promise<void> {
  ...
  const header =
    "time_s,speed_ms,cadence_spm,stride_length_m,total_distance_m,is_running\n";
...
function bindEvents(): void {
  ...
  refs.btnToggleLog.addEventListener("click", () =>
    setPacketLogVisible(!packetLogVisible),
  );
  ...
  refs.scrubRange.addEventListener("input", (event) => {
    ...
    followTail =
      Math.abs(viewStart - getMaxViewStart(VIEW_WINDOW_SECONDS)) < 0.2;
    syncScrubControls();
    scheduleChartRender();
  });
  bindLogResize();
