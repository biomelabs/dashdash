...
import { getAccumStats, getDurationSeconds, getRunStartMs, getSamples } from './run';
...
function refreshConnectionDialog(): void {
    ...
    updateConnectionDialog({
        statusLabel: STATUS_LABELS[connectionState] ?? connectionState.toUpperCase(),
...
export function setConnectButton(connected: boolean): void {
    ...
    refs.btnConnect.className = connected ? BUTTON_CLASSES.danger : BUTTON_CLASSES.primary;
}
...
export function updateMainContentInset(): void {
    const logH = ctx.isPacketLogVisible() ? refs.logArea.getBoundingClientRect().height : 0;
...
export function updateElapsed(): void {
    ...
    const elapsed = runStartMs && bleConnected
...
export function updateSessionStats(shouldFlash = false): void {
    ...
    refs.ssCad.textContent = pkts
        ...
        : '--/--';
    refs.ssSlAvg.textContent = cntSL ? `${(sumSL / cntSL).toFixed(2)} m` : PLACEHOLDER;
...
export function updateLatestStats(
    ...
    refs.vPace.textContent = sample.paceSecPerKm === null
        ? PLACEHOLDER
        : formatPaceSeconds(sample.paceSecPerKm);
    refs.vCadence.textContent = String(sample.cadence);
    refs.vStride.textContent = sample.strideLen !== null
        ? sample.strideLen.toFixed(2)
        : PLACEHOLDER;
    refs.vDistance.textContent = sample.distancePlotM !== null
    ...
    if (shouldFlash) {
        [refs.vSpeed, refs.vPace, refs.vCadence, refs.vStride, refs.vDistance].forEach(flash);
    }
}
