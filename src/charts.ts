...
function makeChart(def: (typeof chartDefs)[ChartKey]): Chart {
  const ctx = (document.getElementById(def.id) as HTMLCanvasElement).getContext(
    "2d",
  )!;
...
export function setChartPreview(charts: Charts, sample: Sample | null): void {
  ...
  for (const chart of Object.values(charts)) {
    const data = chart.data.datasets[0].data as {
      x: number;
      y: number | null;
    }[];
...
export function bindChartPreview(
  charts: Charts,
  {
    onPreview,
    onClear,
  }: { onPreview: (tSec: number) => void; onClear: () => void },
): void {
  for (const chart of Object.values(charts)) {
