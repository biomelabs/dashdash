...
function sampleLogMessage(sample: {
    ...
    return `spd=${sample.speed.toFixed(3)} cad=${sample.cadence}` +
...
function appendLogRow(fragment: DocumentFragment, row: LogRow): void {
    ...
    el.className = 'grid grid-cols-[64px_120px_1fr] gap-2.5 border-b border-border-grid-faint px-3.5 py-0.5 text-[11px] leading-6 max-sm:grid-cols-[52px_1fr]';
