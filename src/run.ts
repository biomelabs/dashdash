...
export async function hydrateRun(): Promise<void> {
    ...
    const lastDistanceSample = [...samples].reverse().find(sample =>
        Number.isFinite(sample.distancePlotM)
    );
...
export function makeSample(
    ...
    const distancePlotM = parsed.totalDist !== null ? parsed.totalDist : lastDistancePlot;
